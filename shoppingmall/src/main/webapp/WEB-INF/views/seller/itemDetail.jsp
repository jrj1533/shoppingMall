...
<style>
	...
	.detail-images-section img {
		width: 100%; /* max-width에서 width로 변경하여 항상 컨테이너 너비에 맞춤(상세이미지 크기) */
		margin: 0 0 30px 0; /* 좌우 auto 마진 제거 */
		height: auto;
		display: block;
		border: 1px solid #ddd;
		border-radius: 5px;
							...
							<c:forEach var="opt" items="${itemOption}">
								<c:choose>
									<c:when test="${opt.stock == 0}">
										<option value="${opt.optionNo}" disabled style="text-decoration: line-through;">${opt.optionName} - ${opt.optionValue} 품절 </option>
									</c:when>
									<c:otherwise>
